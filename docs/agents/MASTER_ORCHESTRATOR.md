...
27. **Arquitectura e Integración del Motor Atómico de Tutoriales In-App (*ShowOnce Tutorial Overlay System*):**
    - **Gestor de Persistencia `TutorialManager.kt` (`utils/`):** Implementación del gestor de persistencia mediante SharedPreferences para el control de visualización única (*ShowOnce*), utilizando claves estandarizadas (`TUTORIAL_HOME_MUNICIPIO`, `TUTORIAL_CREATE_SERVICE_SCHEDULE`, `TUTORIAL_CONTRATACION_HANDSHAKE`, `TUTORIAL_PROFILE_REPUTATION`) que previenen la reaparición no deseada de guías una vez completadas u omitidas.
    ...
    - **Integración Estratégica en Pantallas Clave:**
      ...
      * *`CreateServiceScreen`:* Tutorial paso a paso para la asignación de municipio de cobertura, parametrización de días de atención y el detector de traslapes horarios.
