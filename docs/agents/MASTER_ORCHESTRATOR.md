...
21. **Sincronización Defensiva Automática entre Auth y `public.profiles` (`RegisterUserViewModel` & `LoginViewModel`):**
    - **Registro (`RegisterUserViewModel`):** Tras ejecutar `signUpWith(Email)`, el sistema realiza un `signInWith(Email)` explícito para activar la sesión en tiempo real, obtiene el `userId` y ejecuta `SupabaseManager.client.postgrest["profiles"].upsert(newProfile)` para insertar inmediatamente la fila correspondiente en `public.profiles` con la metadata inicial (`full_name`, `role`, `phone`, `address`, `municipality`).
