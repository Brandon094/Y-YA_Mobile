...
*   **Arquitectura del Motor Atómico de Tutoriales In-App (*ShowOnce Tutorial Overlay System*):**
    ...
    *   *Flujo de Ejecución e Interacción (`TutorialManager -> YayaTutorialTooltip -> YayaTutorialOverlay`):*
        1. **Gestor de Persistencia `TutorialManager.kt` (`utils/`):** Utiliza `SharedPreferences` para verificar el estado de cada tutorial mediante claves estandarizadas de dominio (`TUTORIAL_HOME_MUNICIPIO`, `TUTORIAL_CREATE_SERVICE_SCHEDULE`, `TUTORIAL_CONTRATACION_HANDSHAKE`, `TUTORIAL_PROFILE_REPUTATION`). Si la clave indica que el tutorial ya fue visto o completado (`isTutorialCompleted`), la capa de presentación omite automáticamente la renderización del overlay.
    ...
    *   *Integración en Pantallas Clave:*
        - **`HomeScreen`:** Guía al usuario en el uso del filtro geográfico por municipio (`TUTORIAL_HOME_MUNICIPIO`) y en la exploración por categorías de servicios.
        - **`ProfileScreen`:** Explica la estructura de pestañas (*"Mi Operación"* / *"Ajustes y Ayuda"*) y el acceso al módulo de reputación y reseñas de talento (`TUTORIAL_PROFILE_REPUTATION`).
        - **`CreateServiceScreen`:** Instruye al prestador en la asignación de municipio de atención, días de prestación y la interpretación del detector de traslapes horarios (`TUTORIAL_CREATE_SERVICE_SCHEDULE`).
        - **`ContratacionScreen`:** Orienta al cliente en el agendamiento seguro de citas y el flujo de oferta y contraoferta mediante Handshake digital (`TUTORIAL_CONTRATACION_HANDSHAKE`).
