...
*   **Rediseño del Formulario de Registro en 3 Pasos (Wizard de Registro - `RegisterUserScreen` & `RegisterUserViewModel`):**
    ...
    *   *Paso 1 (Información Personal y Rol):*
        - **Campos:** Nombre Completo (`full_name`), Documento de Identidad / Cédula (`documentId`), Fecha de Nacimiento (`birthDate` DatePicker) y Selección de Rol (*Cliente* vs *Prestador*) mediante tarjetas atómicas de selección (`Surface`/`OutlinedCard`).
        - **Validación de Avance:** Se evalúa reactivamente en tiempo real mediante `isStep1Valid` (exige nombre alfabético sin dígitos, documento de 6 a 12 números, fecha no futura y rol seleccionado).
...
*   **Rediseño del Formulario de Creación de Servicios en 2 Pasos (Wizard de Creación - `CreateServiceScreen` & `CreateServiceViewModel`):**
    ...
    *   *Paso 2 (Precio, Duración, Disponibilidad y Materiales):*
        ...
        - **Duración Estimada Estructurada:** Control compuesto `Row(OutlinedTextField + ExposedDropdownMenuBox)` que combina la cantidad numérica con unidades de medida seleccionables (`Minutos`, `Horas`, `Días`, `Meses`, `Años`), garantizando datos estructurados limpios (ej. `"2 Horas"`).
        - **Insumos/Materiales:** Checkbox con campo condicional para especificar el costo extra de materiales no incluidos.
        - **Días y Horarios de Atención:** Selector de días (`FlowRow`) con botón *"Cargar mi jornada maestra"*, resaltado visual de días ocupados por otros servicios y detector de traslapes, junto con selectores de hora inicio y fin.
...
*   **Control Compuesto de Duración Estimada Estructurada (`CreateServiceScreen`):**
    *   *Composición UI:* Estructura en `Row(OutlinedTextField + ExposedDropdownMenuBox)` donde el `OutlinedTextField` limita la entrada a dígitos numéricos (`isDigit()`) y el `ExposedDropdownMenuBox` expone la lista inmutable de unidades (`Minutos`, `Horas`, `Días`, `Meses`, `Años`).
