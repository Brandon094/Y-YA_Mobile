...
*   **Validación Estricta del Parámetro `role` e Indicador Visual Contextual de Rol (`RegisterUserViewModel` & `RegisterUserScreen`):**
    *   *Validación Estricta de Dominios de Rol (`RegisterUserViewModel`):* El parámetro `role` enviado durante la solicitud de registro es inspeccionado y validado de forma defensiva en `RegisterUserViewModel`, restringiendo su dominio estrictamente a valores válidos autorizados (`client`, `provider`, `admin`). Esta verificación descarta de manera preventiva solicitudes con roles vacíos, no inicializados o malformados antes de interactuar con Supabase Auth y Postgrest.
