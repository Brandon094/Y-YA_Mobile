...
- **Motor Atómico de Tutoriales In-App (*ShowOnce Tutorial Overlay System*):**
    - Implementación de `TutorialManager.kt` (`utils/`) con persistencia en SharedPreferences para el control de visualización única (*ShowOnce*) mediante claves estandarizadas (`TUTORIAL_HOME_MUNICIPIO`, `TUTORIAL_CREATE_SERVICE_SCHEDULE`, `TUTORIAL_CONTRATACION_HANDSHAKE`, `TUTORIAL_PROFILE_REPUTATION`).
    ...
    - Integración de tutoriales interactivos paso a paso en pantallas clave: `HomeScreen` (filtro por municipio y exploración de categorías), `ProfileScreen` (navegación por pestañas y reputación/reseñas de talento), `CreateServiceScreen` (municipio/días de atención y detector de traslapes) y `ContratacionScreen` (agendamiento seguro y negociación Handshake digital).
